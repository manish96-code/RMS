        ...
        <div className="flex items-center justify-between pb-4 border-b border-slate-200">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Review line items, tax breakdown, table assignment and update order status
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="max-w-2xl bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              ...
              {/* Quick Status Control Dropdown for Admin */}
              <div className="flex items-center gap-2">
                <select
                  value={order.status}
                  disabled={isUpdating || order.status === 'completed' || order.status === 'cancelled'}
                  onChange={(e) => handleUpdateStatus(e.target.value)}
                  className="px-3 py-1.5 border border-slate-300 rounded-xl text-xs font-bold text-slate-800 bg-white focus:outline-none"
                >
                ...
                {order.status !== 'completed' && order.status !== 'cancelled' && (
                  <button
                    ...
                    disabled={isUpdating}
                    className="px-3.5 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 font-semibold text-xs rounded-xl transition cursor-pointer disabled:opacity-60"
                  >
                    Cancel Order
                  </button>
                )}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[11px] text-slate-500">
                          ₹{Number(item.price).toFixed(2)} each (Snapshot Price)
                        </div>
                        {item.notes && (
