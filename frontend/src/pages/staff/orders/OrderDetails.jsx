          ...
          <div className="max-w-2xl mx-auto bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-100">
              ...
              <div className="flex items-center gap-2">
                ...
                {order.status === 'served' && (
                  <Link
                    to={`/staff/orders/${order.id}/bill`}
                    className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-2xs transition flex items-center gap-1.5"
                  >
                    <span>🧾</span>
                    <span>Generate Bill & Pay</span>
                  </Link>
                )}

                {order.status === 'completed' && (
                  <Link
                    to={`/staff/orders/${order.id}/receipt`}
                    className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-2xs transition flex items-center gap-1.5"
                  >
                    <span>🖨️</span>
